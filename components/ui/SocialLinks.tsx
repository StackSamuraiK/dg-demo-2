...
const socialIcons = {
    linkedin: {
        icon: ({ width = 24, height = 24, className = '' }: { width?: number; height?: number; className?: string }) => (
...
export default function SocialLinks({
    ...
    return (
        <div className={`flex items-center gap-4 ${className}`}>
            {links.map((link) => {
                const socialConfig = socialIcons[link.name];
                const IconComponent = socialConfig.icon;
                const ariaLabel = link.ariaLabel || socialConfig.defaultLabel;
